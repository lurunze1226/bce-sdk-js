...
var debug = require('debug')('bce-sdk:net');
...
function isPipeName(s) {
  return u.isString(s) && toNumber(s) === false;
}
...
function normalizeConnectArgs(args) {
  ...
  if (u.isObject(args[0])) {
    ...
    options.port = args[0];
    if (u.isString(args[1])) {
      options.host = args[1];
    }
  }
  var cb = args[args.length - 1];
  return u.isFunction(cb) ? [options, cb] : [options];
}
...
function Socket(options) {
  ...
  if (u.isNumber(options)) options = {fd: options}; // Legacy interface.
  else if (u.isUndefined(options)) options = {};
...
Socket.prototype.write = function (chunk, encoding, cb) {
  if (!u.isString(chunk) && !util.isBuffer(chunk)) throw new TypeError('invalid data');
